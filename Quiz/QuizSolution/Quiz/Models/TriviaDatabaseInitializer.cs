namespace GeekQuiz.Models
{
    ...
    using System.Linq;
    using System.Web;
    using Quiz.GeoService;

    public class TriviaDatabaseInitializer : DropCreateDatabaseAlways<TriviaContext>
    {
        //protected void GetCountry(string lblCountry, string txtIPAddress)
        //{
        //    Quiz.GeoService.GeoIPService service = new Quiz.GeoService.GeoIPService();
        //    Quiz.GeoService.GeoIP output = service.GetGeoIP(txtIPAddress);
        //    lblCountry = "Country: " + output.CountryName;
        //}

        protected override void Seed(TriviaContext context)
        {
            //String lblCountry, txtIPAddress;
            int i;

            base.Seed(context);

            var questions = new List<TriviaQuestion>();
            //txtIPAddress = "200.200.55.12";
            //lblCountry = "";

            // for (i = 1; i <= 50; i++)
            //  { 
            //  GetCountry(lblCountry, txtIPAddress);

            // }

            questions.Add(new TriviaQuestion
            {
                Title = "Which states border California?",
